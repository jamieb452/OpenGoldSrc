...
// screen.c -- master for render, status bar, console, chat, notify, etc
...
void SCR_Sky_f ()
{
	...
	if (Cmd_Argc() < 2)
	{
		Con_Printf ("Usage: sky <basename> <rotate> <axis x y z>\n");
		return;
	}
	if (Cmd_Argc() > 2)
...
void SCR_TouchPics ()
{
	...
	if (crosshair->value)
	{
		...
		Q_sprintf (crosshair_pic, sizeof(crosshair_pic), "ch%i", (int)(crosshair->value));
...
void SCR_ExecuteLayoutString (char *s)
{
	...
	while (s)
	{
		...
		{	// draw a ctf client block
			...
			Q_sprintf(block, "%3d %3d %-12.12s", score, ping, ci->name);
...
void SCR_UpdateScreen ()
{
	...
	if (cls.disable_screen)
	{
		...
		{
			cls.disable_screen = 0;
			Con_Printf ("Loading plaque timed out.\n");
		}
		return;
	}
