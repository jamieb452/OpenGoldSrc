...
Copyright (C) 2015-2017 ReHLDS Devs
...
void SV_SendServerinfo_internal(sizebuf_t *msg, client_t *client)
{
	...
	{
		int len = 0;
		byte *mapcyclelist = COM_LoadFileForMe(mapcyclefile.string, &len);
		if (mapcyclelist && len)
		{
...
void EXT_FUNC SV_ConnectClient_internal()
{
	...
	MD5Context_t ctx;
	MD5Init(&ctx);
	MD5Update(&ctx, (byte *)cdkey, sizeof(cdkey));
	MD5Final((byte *)host_client->hashedcdkey, &ctx);
#endif
...
bool EXT_FUNC NET_GetPacketPreprocessor(uint8* data, uint len, const netadr_t& srcAddr)
{
	return true;
}
...
byte* EXT_FUNC SV_FatPVS(float *org)
{
#ifdef REHLDS_FIXES
...
byte* EXT_FUNC SV_FatPAS(float *org)
{
#ifdef REHLDS_FIXES
...
int EXT_FUNC SV_CheckVisibility(edict_t *entity, byte *pset)
{
	int leaf;
...
void EXT_FUNC SV_AddResource(resourcetype_t type, const char *name, int size, byte flags, int index)
{
	resource_t *r;
#ifdef REHLDS_FIXES
