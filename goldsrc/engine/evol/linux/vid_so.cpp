...
void		*reflib_library;		// Handle to render DLL 
...
qboolean VID_LoadRefresh( char *name )
{
	...
	if ((fp = fopen(SO_FILE, "r")) == NULL) {
		Con_Printf( "LoadLibrary(\"%s\") failed: can't open " SO_FILE " (required for location of ref libraries)\n", name);
		return false;
	}
	...
	if (strstr(fn, "softx") == NULL) { // softx doesn't require root
		if (stat(fn, &st) == -1) {
			Con_Printf( "LoadLibrary(\"%s\") failed: %s\n", name, strerror(errno));
			return false;
		}
		if (st.st_uid != 0) {
			Con_Printf( "LoadLibrary(\"%s\") failed: ref is not owned by root\n", name);
			return false;
		}
#if 0
		if ((st.st_mode & 0777) & ~0700) {
			Con_Printf( "LoadLibrary(\"%s\") failed: invalid permissions, must be 700 for security considerations\n", name);
			return false;
		}
#endif
	} else {
	...
	ri.Cvar_Register = Cvar_Register;
...
Con_Printf("Render failed\n");
				sw_mode = Cvar_Get( "sw_mode", "0", 0 );
				if (sw_mode->value != 0) {
Con_Printf("Trying mode 0\n");
					Cvar_SetValue("sw_mode", 0);
					if ( !VID_LoadRefresh( name ) )
						Com_Error (ERR_FATAL, "Couldn't fall back to software render!");
				} else
					Com_Error (ERR_FATAL, "Couldn't fall back to software render!");
			}
...
// This if fake, it's acutally done by the Render load
void IN_Init ()
{
